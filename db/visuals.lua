...
function internal.build.visuals()
	...
	ext:addVisual({
		...
		update = function(self)
			...
			return ExiWoW.Easing.outQuad(delta, 1, -1, duration)*0.5;
		end
	});
	...
	ext:addVisual({
		...
		update = function(self)
			...
			return min(1,max(0,ExiWoW.Easing.outInBounce(delta, 1, -1, duration)))*(0.5+(duration-delta)/2);
		end
	});
	...
	ext:addVisual({
		...
		update = function(self, elapsed)
			...
			return ExiWoW.Easing.inSine(delta, 0.5, -0.5, duration);
		end
	});

	-- /run ExiWoW.require("Visual").get("heavyExcitementPermanent"):trigger()
	-- /run ExiWoW.require("Visual").get("heavyExcitementPermanent"):stop()
	ext:addVisual({
		id="heavyExcitementPermanent",
		image="cloudy_fade_border.tga",
		create = function(self)
